...
class ItemView extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      publish_content: {
        golang: {
          ...
          payload: {
            ...
            GoCode:
              "//Package name that you desire goes down below.\n" +
      ...
      newActivity: {
        state: this.props.location.activity
          ? this.props.location.activity.state
          : null,
        variables: []
      },
      temp_variable: {
  ...
  componentDidMount() {
    ...
    if (candidate && candidate.path) {
      if (
        candidate.path.includes("botmediastorage") &&
        candidate.variables.length
      ) {
    ...
    if (candidate) {
      if (candidate.type === "integration") {
        IntegrationsService.getAllIntegrationConnections(
          this.props.location.activity._id
        )
          .then(res => {
            debugger;
            if (res.data.status) {
  ...
  getFAQ(faq) {
    const _faq = faq.map((f, i) => (
      <AccordionItem key={KEY()} title={f.question} index={"FAQ " + (i + 1)}>
        <p>{f.answer}</p>
      </AccordionItem>
    ));
    return _faq;
  ...
  createVariable = (e, i) => {
    ...
    switch (e.target.id) {
      ...
      case "varIsAdvanced":
        e.target.value === "on"
          ? (this.variable.advance = true)
          : (this.variable.advance = false);
        break;
  ...
  validateVariable = callback => {
    ...
    for (var v in var_) {
      ...
      if (var_[v] === "" || var_[v] === "_") return callback(false, v);
    }
    ...
    if (file) {
      ...
      reader.onload = function(_e) {
        ...
        if (
          ...
          alert(
            "Invalid file format. Please make sure you are uploading a ZIP or a TAR file"
          );
          return;
        } else {
          const _info = [
    ...
    if (this.props.location.activity.type === "activity") {
      ActivitiesService.deleteActivity(name)
        ...
        .catch(errres => {
          ...
          toastr.error(
            "Failed",
            "Activity deleting failed. Please try again later"
          );
          // }
        });
  ...
  init_review = e => {
    ...
    toastr.confirm(
      "You are about to publish your Activity publicaly. First the Activity will be reviewed at Smoothflow and will give you a feedback on the status. You can see all the feedback at Activity Reports. Are you sure you want to send this Activity to review?",
      toastrConfirmOptions
    );
  };
          ...
          ActivitiesService.saveNewActivity(_payload)
            .then(res_ => {
              if (res.status) {
                ...
                toastr.success(
                  "Published to review",
                  "Activity has been successfully submitted to review"
                );
              } else {
                this.props.dispatch(PreloadDialog(false));
                toastr.error(
                  "Error",
                  "Something went wrong when submitting to review"
                );
              }
            })
            .catch(errorres_ => {
          ...
          IntegrationsService.updateIntegration(_payload)
            .then(_res => {
              if (_res.data.IsSuccess) {
                ...
                toastr.error(
                  "Failed",
                  "Failed to add to review. Please try again later."
                );
              }
            })
            .catch(errorres => {
          ...
          <div className="sf-route-content">
            <PageHeader title={this.props.location.activity.name}>
              {this.props.location.activity.state === "private" ? (
                <Button
                  className="sf-button sf-button-primary sf-button-primary-p sf-button-iconed"
                  icon={"cloud_upload"}
                  mat="true"
                  style={{ marginRight: "10px" }}
                  onClick={e => this.init_publish(e)}
                >
                  Publish
                </Button>
              ...
              >
                <Button className="sf-button sf-button-circle">
                  <span className="sf-icon icon-sf_ico_edit"></span>
                </Button>
              </Link>
              {this.props.location.activity.type === "activity" ||
              this.props.location.activity.state === "private" ||
              this.props.location.activity.type === "integration" ? (
                <Button
                  className="sf-button sf-button-circle"
                  onClick={this.deleteInit.bind()}
                >
                  <span className="sf-icon icon-sf_ico_delete"></span>
                </Button>
              ) : null}
            </PageHeader>
                ...
                <Dialog title={"Publish"}>
                  <Block>
                    <div className="sf-flexbox-row">
                      ...
                      <Button
                        className="sf-button sf-button-clear"
                        onClick={this.closeDialog.bind()}
                      >
                        Cancel
                      </Button>
                      <Button
                        className="sf-button sf-button-primary sf-button-primary-p sf-button-caps"
                        type="submit"
                        onClick={e => this.publishActivityPRIVATE(e)}
                      >
                        {" "}
                        Publish{" "}
                      </Button>
                    ...
                    {this.props.uihelper._preload_dialog_ ? (
                      ...
                      <div>
                        {!this.props.location.activity.path.includes(
                          "botmediastorage"
                        ) ? (
                          <div className="sf-flexbox-row">
                            <div
                              ...
                              <div className="sf-p-p-h">
                                <div className="sf-input-block">
                                  <Input
                                    type="radio"
                                    name="publishLang"
                                    className="sf-radiobox"
                                    id="languageNode"
                                    label="Node JS"
                                    value="nodeJs"
                                    onChange={event => this.addInfo(event)}
                                    checked
                                  />
                                </div>
                                <div className="sf-input-block">
                              ...
                              <Wrap>
                                <div className="sf-flex-1 sf-flexbox-column">
                                  <div className="sf-flex-1 sf-p-p sf-flexbox-column">
                                    <label> File </label>
                                    <div
                                      className="sf-card sf-card-block sf-flexbox-column sf-flex-1"
                                      style={{ padding: "15px 0px" }}
                                    >
                                      <div className="sf-card-content sf-card-bordered sf-flex-1 sf-flexbox-column">
                                        <div className="sf-flex-1">
                                          {this.state.publish_content.node
                                            .file ? (
                                        ...
                                        <input
                                          ...
                                          id="publishNode"
                                          onChange={event =>
                                            this.updatePublishContent(event)
                                          }
                                        />
                                      </div>
                                    </div>
                                ...
                                <div className="sf-feature-entry">
                                  <div className="sf-flexbox-row">
                                    <div className="sf-flex-1 sf-guide-holder">
                                      <div
                                        className="sf-input-block sf-flexbox-row"
                                        style={{ marginBottom: "10px" }}
                                      >
                                        <Input
                                          type="checkbox"
                                          id="varIsAdvanced"
                                          value={this.state.newActivity.advance}
                                          onChange={event =>
                                            this.createVariable(event)
                                          }
                                        />
                                        <span>Advance</span>{" "}
                                        <i
                                          className="material-icons sf-icon-guide"
                                          onClick={e =>
                                            this.initHelp(e, "VAR:ADVANCE")
                                          }
                                        >
                                          help
                                        </i>
                                      ...
                                      this.state.active_guide ===
                                        "VAR:ADVANCE" ? (
                                        <Dialog type={"GUIDE"} title="publish">
                                          <div className="sf-dg-header">
                                            <h3>What to do here?</h3>
                                            <i
                                              className="material-icons sf-dg-header-close"
                                              onClick={this.closeHelp.bind()}
                                            >
                                              close
                                            </i>
                                          </div>
                                          <div>
                                            <Block>
                                              This implies whether the Variable
                                              is an <b>Advanced</b> variable or
                                              a <b>Basic</b> variable.
                                              <p>
                                                If you create a Variable with
                                                this value selected, the
                                                Variable will not be visible on
                                                initial( Basic) variables list
                                                at the designer environment.
                                              </p>
                                            </Block>
                                          </div>
                                        </Dialog>
                                  ...
                                  <div className="sf-input-block sf-flexbox-row">
                                    <div className="sf-custom-input sf-flex-1">
                                      <label className="sf-flexbox-row">
                                        Key{" "}
                                        <i
                                          className="material-icons sf-icon-guide"
                                          onClick={e =>
                                            this.initHelp(e, "VAR:KEY")
                                          }
                                        >
                                          help
                                        </i>
                                      ...
                                      />
                                      {this.props.uihelper._init_help &&
                                      this.state.active_guide === "VAR:KEY" ? (
                                        <Dialog type={"GUIDE"} title="publish">
                                          <div className="sf-dg-header">
                                            <h3>What to do here?</h3>
                                            <i
                                    ...
                                    <div className="sf-custom-input sf-flex-1">
                                      <label className="sf-flexbox-row">
                                        ...
                                        <i
                                          className="material-icons sf-icon-guide"
                                          onClick={e =>
                                            this.initHelp(e, "VAR:DISPLAY_NAME")
                                          }
                                        >
                                          help
                                        </i>
                                  ...
                                  <div className="sf-input-block sf-flexbox-row">
                                    <div className="sf-flex-1 sf-custom-input sf-custom-select">
                                      ...
                                      <select
                                        ...
                                        value={
                                          !this.state.temp_variable
                                            .is_val_dropdown &&
                                          !this.state.temp_variable.is_val_api
                                            ? "Textbox"
                                            : this.state.newActivity.control
                                        }
                                      >
                                      ...
                                      this.state.active_guide ===
                                        ...
                                        <Dialog type={"GUIDE"} title="publish">
                                          ...
                                          <div>
                                            <Block>
                                              This implies the behaviour of the
                                              variable when it is in use. If{" "}
                                              <b>Textbox</b> is selected, a text
                                              field will be displayed as the
                                              input component. <b>Dropdown</b>{" "}
                                              will display as a dropdown with
                                              multiple choices.
                                            </Block>
                                          </div>
                                        </Dialog>
                                    ...
                                    <div className="sf-custom-input sf-flex-1">
                                      <label className="sf-flexbox-row">
                                        ...
                                        <i
                                          className="material-icons sf-icon-guide"
                                          onClick={e =>
                                            this.initHelp(e, "VAR:API_METHOD")
                                          }
                                        >
                                          help
                                        </i>
                                          ...
                                          <div>
                                            <Block>
                                              {this.state.temp_variable
                                                .is_val_api ? (
                                                <span>
                                                  Add the URL for the API here.
                                                </span>
                                              ) : (
                                                <span>
                                                  <b>Textbox</b> value to access
                                                  text input.
                                                </span>
                                              )}
                                            </Block>
                                      ...
                                      <div className="sf-flex-1">
                                        <div className="sf-fill-width">
                                          ...
                                          <div className="sf-clearfix">
                                            {this.state.temp_variable.temp_variable_vals.map(
                                              (keyval, index) => (
                                                <div
                                                  className="sf-card"
                                                  key={KEY()}
                                                  ...
                                                  <div className="sf-card-content sf-card-bordered sf-card-centered-row">
                                                    ...
                                                    >
                                                      <div className="sf-txtblock-text">
                                                        <div className="sf-txtblock-txt-title">
                                                    ...
                                                    <div className="sf-card-row-end">
                                                      <button
                                                        type="button"
                                                        className="sf-button sf-button-primary-light sf-button-primary sf-button-circle"
                                                        id="removeVarKeyVal"
                                                        onClick={event =>
                                                          this.createVariable(
                                                            event,
                                          ...
                                          this.state.active_guide ===
                                            ...
                                            >
                                              ...
                                              <div>
                                                <Block>
                                                  You can add the values here
                                                  that come under the
                                                  type(Dropdown) you selected in
                                                  Control section.
                                                </Block>
                                              </div>
                                            </Dialog>
                                          ...
                                          <div className="sf-feature-block">
                                            ...
                                            <div className="sf-feature-add">
                                              <button
                                                type="button"
                                                id="addVarKeyVal"
                                                className="sf-button sf-button-primary sf-button-primary-light"
                                                onClick={event =>
                                                  this.createVariable(event)
                                                }
                                              >
                                  ...
                                  <div className="sf-flexbox-row">
                                    ...
                                    <div className="sf-input-block sf-flex-1 sf-custom-input sf-custom-select">
                                      ...
                                      {this.props.uihelper._init_help &&
                                      this.state.active_guide === "VAR:TYPE" ? (
                                        <Dialog type={"GUIDE"} title="publish">
                                          <div className="sf-dg-header">
                                            <h3>What to do here?</h3>
                                            <i
                                          ...
                                          <div>
                                            <Block>
                                              This implies whether the value is
                                              hardcoded or not. This will be
                                              vary depending on the{" "}
                                              <b>Control</b> type you select.
                                            </Block>
                                          </div>
                                        </Dialog>
                                      ) : null}
                                  ...
                                  <div className="sf-flexbox-row">
                                    <div className="sf-input-block sf-flex-1 sf-custom-input sf-custom-select">
                                      ...
                                      >
                                        ...
                                        <option value="InArgument">
                                          In Argument
                                        </option>
                                        <option value="OutArgument">
                                          Out Argument
                                        </option>
                                      </select>
                                      {this.props.uihelper._init_help &&
                                      this.state.active_guide ===
                                        ...
                                        <Dialog type={"GUIDE"} title="publish">
                                          ...
                                          <div>
                                            <Block>
                                              This implies whether the variable
                                              works as an IN argument or an OUT
                                              argument.
                                            </Block>
                                          </div>
                                        </Dialog>
                                  ...
                                  <div className="sf-flexbox-row">
                                    ...
                                    <button
                                      type="button"
                                      className="sf-button sf-button-primary sf-button-primary-p"
            ...
            <div className="sf-flexbox-row">
              <div className="sf-flex-1">
                <div className="sf-header-bordered">
                  <h3 className="sf-txt-c-p">
                    {this.props.location.activity.name}
                  </h3>
                </div>
              ...
              <div
                className="sf-flex-1 sf-flex-center sf-m-p sf-shadow-box sf-border-radius"
                style={{ display: "flex" }}
              >
                <img
                  src={this.props.location.activity.image}
                  alt=""
                  ...
                  <Tabs>
                    ...
                    >
                      <div className="sf-p-ex sf-auto-fix">
                        {this.getFeatures(
                          this.props.location.activity.features
                        )}
                      </div>
                    </Tab>
                    <Tab
                    ...
                    >
                      <div className="sf-p-ex sf-auto-fix">
                        <Carousel
                          slides={this.props.location.activity.what_you_get}
                        />
                      </div>
                    </Tab>
                    ...
                    <Tab
                      ...
                      title={"Developer"}
                    >
                      <div className="sf-p-ex sf-auto-fix"></div>
                    </Tab>
                    <Tab
                      onClick={e => this.getUserReviewsForActivity(e)}
                      iconClassName={"icon-class-1"}
                      linkClassName={"link-class-1"}
                      title={"Reviews"}
                    >
                    ...
                    <Tab
                      onClick={e => this.getActivityReviews(e)}
                      iconClassName={"icon-class-1"}
                      linkClassName={"link-class-1"}
                    ...
                    >
                      <div className="sf-p-ex sf-auto-fix">
                        {this.props.uihelper._preload_tab_ ? (
                          ...
                          this.props.reviews.active_activity_reviews.map(
                            (comment, i) => (
                              <div
                                className={`sf-comment-block${" sf-comment-" +
                                  comment.status.toLowerCase()}`}
                              >
                                <div className="sf-comment-block-prefix">
                                  <i className="material-icons">
                                    {comment.status === "PENDINGREVISION"
                                      ? "warning"
